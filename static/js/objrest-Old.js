document.addEventListener('DOMContentLoaded', () => {
  ...
  selects.forEach(sel => {    
    sel.addEventListener('change', (event) => {
      ...
      let url = '';
      let pagosUrl = '';    
      let cr = 0; // Conversion Rate     

      if (empleadoId && empleadoId !== 'None') {
          url = `/api/objetivos-y-resultados?empleado_id=${empleadoId}`;
          pagosUrl = `/api/pagos?empleado_id=${empleadoId}`;
        }

      fetch(pagosUrl).then(response => {
        if (!response.ok) {
    ...
    .then(data => {
      ...
      //const totalpagos = pagos || 0;
      const totalpagos = document.getElementById("clientes-value").textContent || 0;
      ...
      mapProductos.forEach(item => {
        ...
        let totalPagosNum = parseFloat(totalpagos) || 0;               
        ...
        // Calcular Conversion Rate (CR), esta parte es la que debe revisarse, no muestra el monto o de manera incorrecta.
        if (totalPagosNum > 0 && sumaPav > 0) {
            cr = ((sumaPav / totalPagosNum) * 100);
            cr = cr.toFixed(2);
        } else {
            cr = 0;
        }
