...
with app.app_context():
    print("=== Checking transacciones table ID sequence ===\n")
    
    # Get the current max ID in the table
    result = db.session.execute(db.text("SELECT MAX(id) FROM transacciones"))
    max_id = result.scalar()
    print(f"Max ID in table: {max_id}")
    
    # Get the current sequence value
    result = db.session.execute(db.text("SELECT last_value FROM transacciones_id_seq"))
    ...
    if max_id and seq_value and max_id >= seq_value:
        ...
        db.session.execute(db.text(f"SELECT setval('transacciones_id_seq', {max_id + 1}, false)"))
        db.session.commit()
